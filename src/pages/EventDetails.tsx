...
const EventDetails: React.FC<Props> = ({ route }) => {
  ...
  const handleContact = (type: 'call' | 'text') => {
    ...
    Linking.canOpenURL(phoneUrl)
      ...
      .catch(() => Alert.alert('Error', `Failed to ${type} the organizer. Please try again.`));
  };
  ...
  const handleShowRoute = () => {
    ...
    if (url) {
      Linking.canOpenURL(url)
        ...
        .catch(() => Alert.alert('Error', 'Failed to open the map. Please try again.'));
    }
  };

  const getStatusText = () => {
    if (eventId === PAST_EVENT_ID) return 'Volunteered';
    if (currentUser && event.volunteersIds.includes(currentUser.id)) return 'You have already volunteered';
    if (event.volunteersIds.length >= event.volunteersNeeded) return 'Team is full';
